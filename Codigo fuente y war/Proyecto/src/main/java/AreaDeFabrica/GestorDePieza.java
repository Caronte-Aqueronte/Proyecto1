...
public class GestorDePieza {
    ...
    public String insertarPieza(Pieza pieza) {
        ...
        try {
            ...
            PreparedStatement query1 = CONEXION.prepareStatement(
                    "SELECT * FROM pieza WHERE UPPER(nombre) = (?) AND precio = ?;");
            ...
            if (suma >= 0) {
                if (contadorElementoExistente > 0) {
                    ...
                    PreparedStatement query2 = CONEXION.prepareStatement(
                            "UPDATE pieza SET existencias = ? WHERE UPPER(nombre) = UPPER(?) AND precio = ?;");
    ...
    public void eliminarPieza(Pieza pieza){
        try {
            ...
            PreparedStatement query = CONEXION.prepareStatement(
                    "DELETE FROM pieza WHERE UPPER(nombre) = UPPER(?) AND precio = ?;");
    ...
    public boolean haySuficientesPiezas(String nombreDePieza, double precioDePieza, int piezasQueSeNecesitan) {
        try {
            PreparedStatement query = CONEXION.prepareStatement(
                    "SELECT * FROM pieza WHERE UPPER(nombre) = UPPER(?) and precio = ?;");
    ...
    public static boolean saberSiExistePieza(String nombre, double precio){
        try{
            PreparedStatement query = CONEXION.prepareStatement(//esta query devuelve todas las tuplas con caracteristicas iguaes
            "SELECT * FROM pieza WHERE UPPER(nombre) =  UPPER(?) AND precio = ?");
            query.setString(1, nombre);//damos valores a los ?
            query.setDouble(2, precio);
            ResultSet resultado = query.executeQuery();//ejecutamos la query
            while(resultado.next()){//si entra entonces existe
                return true;
            }
            return false;//si llega aqui etonces no existe
        }catch(SQLException ex){
            return false;
        }
    }

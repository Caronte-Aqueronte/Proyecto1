...
public class GestorDeVenta {
        ...
        try {
            ...
            for (ProductoEnVenta item : venta) {
                if (!codigoCliente.equals("") && item.getCodigoDeEnsamble() != 0) {
                    codigoDeEnsamble = item.getCodigoDeEnsamble();//obtenemos el codigo de ensamble que devolvemos
                    nombreMueble = item.getCodigoMueble();//obtenemos el nombre del mueble que vendemos
                    precioMueble = saberPrecioMueble(nombreMueble);//obtenemos el precio del mueble que vendemos
                    query = CONEXION.prepareStatement("UPDATE stock SET estado = ? WHERE UPPER(codigo_de_ensamble) = UPPER(?) ");
                    ...
                    query.setDouble(5, precioMueble);
                    query.executeUpdate();
                } else {
    ...
    public double saberPrecioMueble(String codigoMueble){
        try{
            PreparedStatement query = CONEXION.prepareStatement(
            "SELECT precio FROM mueble WHERE UPPER(nombre) = UPPER(?);");
            query.setString(1, codigoMueble);
    ...
    public ResultSet buscarNit(String nit) {
        try {
            PreparedStatement query = CONEXION.prepareStatement(
                    "SELECT * FROM cliente WHERE UPPER(nit) = UPPER(?)");
    ...
    public String hacerDevolucion(String codigoDeCliente, String motivo, int codigoDeFactura, int codigoDeEnsamble) {
        try {
            ...
            PreparedStatement query = CONEXION.prepareStatement(
                    "SELECT * FROM compra WHERE codigo_de_factura = ? AND UPPER(codigo_de_cliente) = UPPER(?)"); //seleccionamos la factura con ese codigo
            ...
            if (bandera == true) {//si la bandera es true entonces existe el codigo y seguimos
                ...
                query = CONEXION.prepareStatement(
                        "SELECT * FROM compra WHERE codigo_de_factura = ? AND UPPER(codigo_de_ensamble) = UPPER(?)");
                query.setInt(1, codigoDeFactura);
                query.setInt(2, codigoDeEnsamble);
